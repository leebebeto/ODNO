...
class Ui_Form(object):
	...
	def object_detection_api(self, data_path, threshold=0.5, rect_th=2, text_size=1.5, text_th=2): 
		print(self.blurry_people_button.isChecked())
		print(self.blurry_boxes_button.isChecked())
		print(self.face_key_button.isChecked())
		...
		if self.buttons[self.photo_button] == 1:
			...
			if self.blurry_boxes_button.isChecked() and self.biggest_button.isChecked():
			...
			elif self.blurry_people_button.isChecked() and self.biggest_button.isChecked():
				print('people and biggest')
				blurred_img  = img
			...
			elif self.blurry_people_button.isChecked() and self.face_key_button.isChecked():
				print('people and face-key')
				img = cv2.imread(img_path)
				...
				self.resultLayout.addWidget(self.label, 0, QtCore.Qt.AlignHCenter)
				self.resultLayout.addWidget(self.label, 0, QtCore.Qt.AlignHCenter)
			elif self.blurry_boxes_button.isChecked() and self.face_key_button.isChecked():
				print('box and face-key')
				print('not yet')

			else:
				print("wrong option in photo conversion")
				...
				for i in range(len(box_size_dict)):
					...
					if i ==0:       
						...
						try:
							...
							print('other i error')
				if self.blurry_boxes_button.isChecked() and self.biggest_button.isChecked():
					cv2.imwrite(image2blurry_finished_folder+'/'+'revised%d',idx, img)
					print('box and biggest')
			...
			filenames = os.listdir(video2image_path)
			image_list = []
			save_path = join(video_folder_path,'image2gif_folder/')
			iteration = 0
			for filename in filenames:
				with open(video2image_path+filename, 'rb') as data_file:
					try:
						image_list.append(imageio.imread(data_file))
					except:
						pass
			imageio.mimsave(save_path+'/movie.gif', image_list)
	...
	def upload_button_clicked(self):  
		...
		self.conversion_button.clicked.connect(partial(self.object_detection_api, self.data_path, threshold=0.8))
	...
	def reset_button_clicked(self):
		...
		self.upload_button.setParent(None)
		self.label.setParent(None)
		self.blurry_people_button.setParent(None)
		self.blurry_boxes_button.setParent(None)
		self.face_key_button.setParent(None)
		self.biggest_button.setParent(None)
		...
		self.blurry_people_button.setChecked(False)
