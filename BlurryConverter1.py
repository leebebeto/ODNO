...
class Ui_Form(object):
	...
	def object_detection_api(self, img_path, threshold=0.5, rect_th=2, text_size=1.5, text_th=2): 
		...
		for i in range(len(box_size_dict)):
			...
			if i ==0:       
				...
				try:
					...
					print('other i error')
		if self.buttons[self.photo_button] == 1 and self.blurry_boxes_button.isChecked() and self.biggest_button.isChecked():
		...
		elif self.buttons[self.photo_button] == 1 and self.blurry_people_button.isChecked() and self.face_key_button.isChecked():
			img = cv2.imread(img_path)
